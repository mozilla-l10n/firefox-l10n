...
identity-https-only-label2 = Автоматически обновить этот сайт для безопасного соединения
...
identity-https-only-info-turn-on3 = Включите обновление на HTTPS для этого сайта, если хотите, чтобы { -brand-short-name } обновлял соединение, когда это возможно.
identity-https-only-info-turn-off3 = Если страница кажется неработающей, вы можете отключить обновления на HTTPS для этого сайта, чтобы перезагрузить его с использованием небезопасного HTTP.
