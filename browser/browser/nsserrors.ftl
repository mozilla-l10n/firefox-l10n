...
psmerr-ssl2-disabled = Не удалось установить защищённое соединение, так как сайт использует устаревшую, небезопасную версию протокола SSL.
...
sec-error-no-memory = библиотека безопасности: неудача распределения памяти.
