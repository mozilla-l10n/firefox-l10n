...
sec-error-inadequate-cert-type = Kiểu chứng chỉ không được chấp thuận trong ứng dụng.
...
sec-error-pkcs12-unable-to-locate-object-by-name = Không thể xuất. Không thể xác định vị trí chứng chỉ hoặc khóa bằng biệt danh.
...
sec-error-cert-not-in-name-space = Nhà thẩm định của chứng chỉ này không được phép cấp phát chứng chỉ có tên như vậy.
...
sec-error-unknown-signer = Không tìm thấy chứng chỉ của người kí.
...
sec-error-revoked-certificate-crl = Chứng chỉ bị thu hồi trong danh sách thu hồi chứng chỉ của nhà cấp phát.
sec-error-revoked-certificate-ocsp = Phản hồi OCSP của nhà cấp phát báo rằng chứng chỉ đã bị thu hồi.
sec-error-crl-invalid-version = Danh sách Thu hồi Chứng chỉ của nhà Cấp phát có số phiên bản không xác định.
sec-error-crl-v1-critical-extension = Danh sách Thu hồi Chứng chỉ V1 của nhà Cấp phát có phần gia hạn nghiêm trọng.
sec-error-crl-unknown-critical-extension = Danh sách Thu hồi Chứng chỉ V2 của nhà Cấp phát có phần gia hạn nghiêm trọng chưa xác định.
...
sec-error-ocsp-responder-cert-invalid = Chứng chỉ của phần phản hồi OCSP được cấu hình không hợp lệ.
sec-error-ocsp-bad-signature = Phản hồi OCSP có một chữ kí bất hợp lệ.
sec-error-out-of-search-limits = Tìm kiếm kiểm định chứng chỉ bị ngoài giới hạn tìm kiếm
sec-error-invalid-policy-mapping = Định tuyến chính sách chứa anypolicy
sec-error-policy-validation-failed = Chuỗi chứng chỉ bị thất bại khi kiểm định chính sách
sec-error-unknown-aia-location-type = Kiểu địa điểm chưa biết trong phần mở rộng AIA chứng chỉ
...
sec-error-bad-info-access-location = Địa điểm truy cập thông tin xấu trong mở rộng chứng chỉ
sec-error-libpkix-internal = Lỗi nội bộ Libpkix xảy ra trong quá trình kiểm định chứng chỉ.
...
sec-error-bad-info-access-method = Không rõ phương thức truy cập thông tin trong mở rộng chứng chỉ.
...
sec-error-cert-signature-algorithm-disabled = Chứng chỉ được ký theo thuật toán ký đã bị hủy bỏ do không an toàn.
...
mozilla-pkix-error-ca-cert-used-as-end-entity = Máy chủ sử dụng chứng chỉ với phần mở rộng ràng buộc cơ bản xác định nó là cơ quan cấp chứng chỉ. Đối với một chứng chỉ được cấp đúng, điều này không nên xảy ra.
mozilla-pkix-error-inadequate-key-size = Máy chủ xuất trình chứng chỉ với kích thước khóa quá nhỏ để thiết lập kết nối an toàn.
mozilla-pkix-error-v1-cert-used-as-ca = Chứng chỉ X.509 phiên bản 1 không được tin cậy đã được sử dụng để cấp chứng chỉ máy chủ. Chứng chỉ X.509 phiên bản 1 không được dùng nữa và không nên được sử dụng để ký các chứng chỉ khác.
mozilla-pkix-error-not-yet-valid-certificate = Máy chủ xuất trình chứng chỉ chưa hợp lệ.
mozilla-pkix-error-not-yet-valid-issuer-certificate = Chứng chỉ chưa hợp lệ đã được sử dụng để cấp chứng chỉ máy chủ.
mozilla-pkix-error-signature-algorithm-mismatch = Thuật toán chữ ký trong trường chữ ký của chứng chỉ không khớp với thuật toán trong trường signatureAlgorithm.
mozilla-pkix-error-ocsp-response-for-cert-missing = Phản hồi OCSP đã không bao gồm trạng thái cho chứng chỉ được xác minh.
mozilla-pkix-error-validity-too-long = Máy chủ xuất trình chứng chỉ có giá trị quá dài.
mozilla-pkix-error-required-tls-feature-missing = Một tính năng bắt buộc của TLS bị thiếu.
mozilla-pkix-error-invalid-integer-encoding = Máy chủ xuất trình chứng chỉ chứa mã hóa không hợp lệ của một số nguyên. Các nguyên nhân phổ biến bao gồm số sê-ri âm, mô-đun RSA âm và mã hóa dài hơn mức cần thiết.
mozilla-pkix-error-empty-issuer-name = Các máy chủ xuất trình một chứng chỉ với một tên phân biệt nhà phát hành trống.
mozilla-pkix-error-additional-policy-constraint-failed = Một ràng buộc của chính sách bổ sung không thành công khi xác nhận chứng chỉ này.
mozilla-pkix-error-self-signed-cert = Chứng chỉ này không đáng tin vì nó được tự kí.
