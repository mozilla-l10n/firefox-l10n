...
ssl-error-no-certificate = Klarte ikkje å finne sertifikatet eller nykelen som er nødvendig for å autentisere.
...
ssl-error-extract-public-key-failure = SSL klarte ikkje å pakka ut den offentlege nykelen frå sertifikatet til motparten.
...
ssl-error-sym-key-context-failure = Klarte ikkje å opprette symmetrisk nykel-innhald.
ssl-error-sym-key-unwrap-failure = Klarte ikkje å pakk ut symmetrisk nykel i Client Key Exchange-meldinga.
ssl-error-pub-key-size-limit-exceeded = SSL-tenaren prøvde å bruk innanriks-grads offentleg nykel med eksport-chiffreringssamling.
...
ssl-error-cert-kea-mismatch = Det mottekne sertifikatet kan ikkje brukast med den valde nykelutvekslingsalgoritmen.
...
ssl-error-weak-server-cert-key = Tenar-sertifikatet inneheld ein offentlig nykel som er for svak.
...
sec-error-bad-key = Den offentlege nykelen til motpartens er ugyldig.
...
sec-error-invalid-key = Nykelen støttar ikkje den etterspurde operasjonen.
...
sec-error-not-a-recipient = Klarte ikkje å dekryptere: Du er ikkje ein mottaker, eller tilsvarande sertifikat og privat nykel ikkje funne.
sec-error-pkcs7-keyalg-mismatch = Klarte ikkje å dekryptere: Nykelen sin krypteringsalgoritme stemmer ikkje med sertifikatet.
...
sec-error-decryption-disallowed = Klarte ikkje å dekryptere: Er kryptert med ein algoritme eller nykelstorleik som ikkje er tillaten.
...
sec-error-revoked-key = Nykelen for sertifikatet åt denne nettstaden er tilbakekalt.
...
sec-error-pkcs12-unable-to-import-key = Klarte ikkje å importere. Feil under import av privat nykel.
sec-error-pkcs12-importing-cert-chain = Klarte ikkje å importere. Feil ved import av sertifikatkjede.
sec-error-pkcs12-unable-to-locate-object-by-name = Klarte ikkje å eksportere. Klarte ikkje å finne sertifikatet eller nykel på kallenamnet.
sec-error-pkcs12-unable-to-export-key = Klarte ikkje å eksportere. Privat nykel vart ikkje funnen og eksportert.
...
sec-error-pkcs12-key-database-not-initialized = Klarte ikkje å eksportere. Nykeldatabasen er ugyldig eller sletta.
sec-error-keygen-fail = Klarte ikkje å opprette offentleg/privat nykelpar.
