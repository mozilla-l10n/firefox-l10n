...
policy-Preferences = Устанавіць і зафіксаваць значэнне для набору налад.
policy-PromptForDownloadLocation = Пытаць, куды захаваць файлы, пры сцягванні.
policy-Proxy = Наладзіць параметры проксі.
policy-RequestedLocales = Усталяваць спіс запытаных моў для праграмы ў жаданым парадку.
