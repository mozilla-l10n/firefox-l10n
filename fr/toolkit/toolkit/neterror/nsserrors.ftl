...
ssl-error-decompression-failure-alert = Le pair SSL n’a pas réussi à décompresser un enregistrement SSL qu’il a reçu.
...
sec-error-bad-template = Impossible de décoder les données ASN.1. Le modèle spécifié était invalide.
...
sec-error-unsupported-ec-point-form = Forme de point de courbe elliptique non prise en charge.
