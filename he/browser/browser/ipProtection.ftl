...
ipprotection-connection-status-paused-title-1 = ה־VPN מושהה
# Variables
#   $maxUsage (number) - The maximum amount of data a user can use in a month (in GB)
...
ip-protection-not-opted-in-3 =
    .heading = לנסות את ה־VPN המובנה של { -brand-short-name }
    .message = ניתן לגלוש עם הגנה נוספת על־ידי הסתרת המיקום שלך. נסו { $maxUsage } ג״ב של נתוני VPN חינמיים בכל חודש.
