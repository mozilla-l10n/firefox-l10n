...
ssl-error-no-cypher-overlap = Eş ile güvenli iletişim kurulamadı: Ortak şifreleme algoritmaları yok.
...
ssl-error-bad-client = Sunucu, istemciden gelen bozuk veri ile karşılaştı.
ssl-error-bad-server = İstemci, sunucudan gelen bozuk veri ile karşılaştı.
...
ssl-error-sym-key-unwrap-failure = Client Key Exchange iletisindeki simetrik anahtar paketi açılamadı.
...
ssl-error-no-server-key-for-alg = Sunucu, denenen anahtar değişim algoritması için anahtara sahip değil.
...
ssl-error-server-cache-not-configured = SSL sunucu önbelleği bu soket için ayarlanmadı ve devre dışı bırakılmadı.
...
ssl-error-rx-unexpected-uncompressed-record = SSL beklenmeyen bir sıkıştırılmamış kayıt aldı.
...
sec-error-invalid-algorithm = güvenlik kitaplığı: geçersiz algoritma.
...
sec-error-no-memory = güvenlik kitaplığı: bellek ayırma hatası.
...
sec-error-unsupported-keyalg = Desteklenmeyen veya bilinmeyen anahtar algoritması.
sec-error-decryption-disallowed = Şifre çözülemiyor: İzin verilmeyen bir algoritma veya anahtar boyutuyla şifrelenmiş.
...
sec-error-krl-bad-signature = Bu sitenin sertifikasının KRL imzası geçersiz.
...
sec-error-no-slot-selected = Yuva veya jeton seçilmedi.
...
sec-error-bad-export-algorithm = Gerekli algoritmaya izin verilmiyor.
...
sec-error-not-fortezza-issuer = FORTEZZA eş zinciri FORTEZZA olmayan sertifika içeriyor.
sec-error-cannot-move-sensitive-key = Duyarlı bir anahtar ihtiyaç duyulan yuvaya taşınamıyor.
sec-error-js-invalid-module-name = Geçersiz modül adı.
sec-error-js-invalid-dll = Geçersiz modül yolu veya dosya adı
...
sec-error-ocsp-malformed-request = OCSP sunucusu isteğin bozulmuş veya düzgün olmayan biçimde olduğunu fark etti.
...
sec-error-module-stuck = PKCS #11 modülü hâlâ kullanımda olduğu için kaldırılamıyor.
...
sec-error-out-of-search-limits = Sertifika doğrulama araması arama sınırlarının dışında
...
sec-error-bad-info-access-method = Sertifika uzantısında bilinmeyen bilgiye erişim yöntemi.
...
mozilla-pkix-error-validity-too-long = Sunucu, geçerliliği fazla uzun bir sertifika sundu.
...
xp-java-delete-privilege-error = Ayrıcalık silinemedi
