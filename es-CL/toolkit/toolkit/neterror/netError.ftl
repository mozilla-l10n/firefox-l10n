...
neterror-search-cta-intro2 = No se pudo conectar con el servidor en <strong>{ $hostname }</strong>.
# $domain (String) - The host of the address that failed to load, including any
# subdomains, for example "docs.example.com".
neterror-search-cta-intro = No se pudo conectar con el servidor en { $domain }.
# Heading above the list of recovery hints on the search CTA error page.
neterror-search-cta-things-to-try = Prueba estos pasos:
neterror-search-cta-hint-check-address = Verifica dos veces la dirección del sitio web
# Shown when no Search button is offered, so no specific query can be named.
neterror-search-cta-hint-search = Busca en la web para encontrar el sitio
...
neterror-search-cta-hint-search-query = Busca en la web <strong>“{ $query }”</strong>
# .tooltiptext doubles as the button's accessible description, so it explains
# that results open in a new tab.
neterror-search-cta-search-button =
    .label = Buscar
    .tooltiptext = Abre los resultados de la búsqueda en una pestaña nueva
    .accesskey = B
...
neterror-search-cta-offline = Parece que estás sin conexión. Vuelve a conectarte e inténtalo de nuevo.
...
neterror-search-cta-learn-more = Más información
...
neterror-sslkeylogging-warning =
    .heading = Tu conexión podría no ser privada
    .message = Una app o servicio podría ver tu tráfico cifrado desde este sitio.
...
neterror-http-empty-response = Verifica que hayas escrito correctamente la dirección del sitio web y vuelve a intentarlo en unos momentos.
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
neterror-http-empty-response-description = { $hostname } envió una página vacía.
...
neterror-dns-not-found-hint-check-network-2 = Revisa tu conexión de red.
neterror-dns-not-found-hint-firewall-2 = Verifica que { -brand-short-name } tenga permiso para acceder a la web (es posible que tengas conexión pero estés detrás de un firewall).
...
neterror-unknown-socket-type-client-config = Esto podría deberse a un error de configuración con tu cliente.
...
fp-neterror-content-encoding-intro = La página que estás intentando ver no puede ser mostrada porque utiliza un formato de compresión no válido o no admitido.
...
fp-learn-more-about-https-connections = Aprende acerca de las conexiones HTTPS
fp-neterror-vpn-error-title = No se pudo conectar a la VPN
fp-neterror-vpn-error-description = Vuelve a intentarlo en unos minutos.
fp-neterror-denied-port-access = Esta dirección usa un puerto de red que normalmente se utiliza con fines distintos a la navegación web. Para tu protección, { -brand-short-name } ha cancelado la petición.
