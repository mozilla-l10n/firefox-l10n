...
neterror-http-empty-response = 請檢查您輸入的網址是否正確，並稍後再試。
# Variables:
...
neterror-dns-not-found-hint-check-network-2 = 檢查網際網路連線是否正常。
neterror-dns-not-found-hint-firewall-2 = 檢查 { -brand-short-name } 是否有權限開啟網頁（可能已經連上網路，但被防火牆阻擋）。
