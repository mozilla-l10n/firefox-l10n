...
ssl-error-iv-param-failure = PKCS11 kod nije uspio prevesti IV u parametar.
...
ssl-error-session-not-found = Klijentov ID SSL prijave nije pronađen u predmemoriji prijava servera.
...
ssl-error-server-cache-not-configured = Predmemorija SSL servera nije podešena i nije deaktivirana za ovaj priključak.
...
sec-error-ocsp-try-server-later = OCSP server predlaže da kasnije pokušaš ponovo.
