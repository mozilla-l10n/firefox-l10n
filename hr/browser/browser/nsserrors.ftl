...
ssl-error-post-warning = Nepoznata šifra SSL greške.
...
ssl-error-bad-cert-alert = SSL čvor ne može ovjeriti vaš certifikat.
ssl-error-revoked-cert-alert = SSL čvor je odbio vaš certifikat kao poništeni.
ssl-error-expired-cert-alert = SSL čvor je odbio vaš certifikat kao istekli.
...
ssl-error-bad-block-padding = SSL je primio zapis s lošom blok postavom.
...
ssl-error-sign-hashes-failure = Digitalno potpisivanje podataka potrebnih za ovjeru vašeg certifikata nije moguće.
...
ssl-error-iv-param-failure = PKCS11 kôd nije uspio prevesti IV u parametar.
...
ssl-error-unknown-ca-alert = Čvor ne prepoznaje i ne vjeruje vašem izdavaču certifikata.
ssl-error-access-denied-alert = Čvor je primio valjani certifikat, ali pristup je odbijen.
ssl-error-decode-error-alert = Čvor nije mogao dešifrirati SSL poruku rukovanja.
...
ssl-error-certificate-unobtainable-alert = SSL čvor nije mogao dobiti vaš certifikat iz ponuđenog URL-a.
...
sec-error-duplicate-cert-name = Preuzeti naziv certifikata se podudara s drugim koji se već nalazi u vašoj bazi podataka.
...
sec-error-expired-issuer-certificate = Certifikat izdavača certifikata je istekao. Provjerite datum i vrijeme na vašem sustavu.
sec-error-crl-expired = CRL za izdavača certifikata je istekao. Ažurirajte ga ili provjerite datum i vrijeme na vašem sustavu.
...
sec-error-no-recipient-certs-query = Nije šifrirano: nemate certifikate za svakog od primatelja.
sec-error-not-a-recipient = Dešifriranje nije moguće: vi niste primatelj ili odgovarajući certifikat i privatni ključ nisu pronađeni.
sec-error-pkcs7-keyalg-mismatch = Dešifriranje nije moguće: algoritam šifriranja ključa ne odgovara vašem certifikatu.
...
xp-sec-fortezza-bad-card = Fortezza kartica nije ispravno inicijalizirana. Uklonite ju i vratite vašem izdavaču.
...
sec-error-reused-issuer-and-serial = Pokušavate uvesti certifikat s istim izdavačem/serijskim brojem kao postojeći certifikat, ali radi se o različitom certifikatu.
sec-error-busy = NSS nije moguće isključiti. Objekti su još uvijek u upotrebi.
sec-error-extra-input = DER-šifrirana poruka sadrži dodatne neiskorištene podatke.
