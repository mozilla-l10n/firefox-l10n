...
title = Tocante al tráficu de rede
warning = Esto ye peresperimental. Nun usar ensin supervisión d'un adultu.
show-next-time-checkbox = Amosar esti avisu la próxima vegada
ok = Aceutar
http = HTTP
sockets = Sockets
dns = DNS
...
refresh = Refrescar
auto-refresh = Refrescar cada 3 segundos
...
received = Recibióse
family = Familia
addresses = Direiciones
expires = Caduca (segundos)
messages-sent = Mensaxes unviaos
messages-received = Mensaxes recibíos
bytes-sent = Bytes unviaos
bytes-received = Bytes recibíos
logging = Rexistru
log-tutorial = Mira <a data-l10n-name="logging">Rexistru HTTP</a> pa instrucciones tocante a cómo usar esta ferramienta.
current-log-file = Ficheru actual de rexistru:
current-log-modules = Módulos actuales de rexistru:
set-log-file = Afitar ficheru de rexistru
set-log-modules = Afitar módulos de rexistru
start-logging = Aniciar rexistru
stop-logging = Parar rexistru
dns-lookup = Gueta de DNS
dns-lookup-button = Resolver
dns-domain = Dominiu:
dns-lookup-table-column = IPs
rcwn = Estadístiques RCWN
rcwn-status = Estáu RCWN
rcwn-cache-won-count = Cantidá de caché ganada
rcwn-net-won-count = Cantidá de rede ganada
total-network-requests = Cantidá de solicitúes totales de rede
rcwn-operation = Operación de caché
rcwn-perf-open = Apertura
rcwn-perf-read = Llectura
rcwn-perf-write = Escritura
rcwn-perf-entry-open = Apertura d'entraes
rcwn-avg-short = Promediu curtiu
rcwn-avg-long = Promediu llargu
rcwn-std-dev-long = Esviación llarga estándar
rcwn-cache-slow = Cantidá de caché lenta
rcwn-cache-not-slow = Cantidá de caché non lenta
