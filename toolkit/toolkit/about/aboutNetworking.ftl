...
title = Mu dhèidhinn lìonraidhean
warning = Tha seo air leth deuchainneil, na cleachd seo gun inbheach ri do thaobh.
show-next-time-checkbox = Seall an rabhadh seo an ath-thuras
ok = Ceart ma-tha
http = HTTP
sockets = Socaidean
dns = DNS
...
auto-refresh = Ath-nuadhaich a h-uile 3 diogan
...
messages-received = Teachdaireachdan a fhuaradh
bytes-sent = Bytes a chaidh a chur
bytes-received = Bytes a fhuaradh
logging = Logadh
log-tutorial = Faic <a data-l10n-name="logging">Logadh HTTP</a> airson stiùireadh mun inneal seo.
current-log-file = Am faidhle logaidh làithreach:
current-log-modules = Na mòidealan logaidh làithreach:
set-log-file = Suidhich faidhle logaidh
set-log-modules = Suidhich mòidealan logaidh
start-logging = Tòisich air logadh
stop-logging = Sguir dhen logadh
dns-lookup = Lorg DNS
dns-lookup-button = Fuasgail
dns-domain = Àrainn:
dns-lookup-table-column = IPan
rcwn = RCWN Stats
