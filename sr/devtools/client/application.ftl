...
sidebar-item-session-history = Историјат сесије
    .alt = Иконица историјата сесије
    .title = Историјат сесије
# Entry in the Session History diagram
session-history-entry-info-button-title =
    .title = Прикажи податке историјата сесије
...
session-history-navigate-button-title = Иди до уноса у историји сесије { $index }
# Header for the Session History page when session history diagrams are unavailable
session-history-unavailable = Дијаграм историје сесије није доступан
# Text displayed for when the target does not support showing session history diagrams
session-history-target-unsupported = Мета не подржава приказ историјских дијаграма сесије
