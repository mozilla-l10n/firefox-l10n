...
place-database-last-idle-maintenance-data = Fecha de último mantenimiento por inactividad
# Vacuum refers to a type of database maintenance process
place-database-last-vacuum-date = Fecha de última limpieza
place-database-last-integrity-corruption-date = Fecha de última corrupción de integridad
...
pdfjs-version = Versión de PDF.js
pdfjs-enabled = Habilitado
# The annotation editor is the set of PDF editing tools (draw, add image, add
# text, signature).
pdfjs-enabled-annotation-editor = Editor de anotaciones habilitado
# XFA (XML Form Architecture) refers to Adobe’s technology for forms.
pdfjs-enabled-xfa = Formularios XFA habilitados
pdfjs-open-attachments-inline = Abrir adjuntos PDF integrados
