...
fxa-menu-signed-out-description = Has cerrado sesión
...
sync-profile-different-account-description-merge = Para mantener los datos organizados y seguros, recomendamos crear un nuevo perfil para iniciar sesión usando { $acctEmail }. Si prefieres continuar la sincronización en este perfil, los datos de ambas cuentas se combinarán permanentemente en “{ $profileName }”.
# Dialog 2 - account signed in on another profile without option to merge
sync-account-in-use-header = La cuenta ya se encuentra en uso
# Variables:
#   $acctEmail (String) - Email of the account signing into sync.
#   $otherProfile (String) - Name of the other profile that is associated with the account
sync-account-in-use-header-merge = { $acctEmail } ya inició sesión en el perfil “{ $otherProfile }”
sync-account-in-use-description = Solo puedes asociar esta cuenta con un perfil en esta computadora.
