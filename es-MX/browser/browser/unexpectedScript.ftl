...
unexpected-script-load-detail-1-block = { -brand-short-name } bloqueará la carga de scripts privilegiados inesperados, incluido el siguiente. Esto hará que su instalación de { -brand-short-name } sea más segura.
...
unexpected-script-load-detail-2 = Incluso si permite este script, debes de informar { -vendor-short-name } para ayudar a comprender cómo y por qué se cargó. Sin esta información, la funcionalidad dejará de funcionar en el futuro.
...
unexpected-script-load-email-checkbox =
    .label = Incluye mi correo electrónico para que { -vendor-short-name } puedan contactarme si es necesario
unexpected-script-load-email-textbox =
    .placeholder = Introduce tú correo electrónico aquí
    .aria-label = Introduce tú correo electrónico aquí
unexpected-script-load-more-info = Más información
unexpected-script-load-learn-more = Saber más
unexpected-script-load-telemetry-disabled = Los informes están deshabilitados porque la telemetría está deshabilitada en Configuración. Habilita la telemetría para generar informes.
