...
settings-data-toggle-encryption-label2 =
    .label = 機密性の高い情報をバックアップする
    .description = パスワードや支払い方法を保護し、すべてのユーザーデータを暗号化して安全に保管します。
# Variables:
...
turn-on-scheduled-backups-error-default-dir-denied = バックアップフォルダーにアクセスできませんでした。別の場所を選択してください。
...
restore-from-backup-type-group-label =
    .label = 現在のプロファイルを置き換えますか？
restore-from-backup-type-replace =
    .label = すべてのデータをこのバックアップと置き換える
restore-from-backup-type-add =
    .label = データを変更せず新しいプロファイルを作成する
restore-from-backup-profiles-disabled-message = この操作は現在のすべての { -brand-short-name } データをバックアップしたデータと置き換えます。
