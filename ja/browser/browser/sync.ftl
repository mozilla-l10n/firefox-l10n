...
sync-profile-different-account-description-merge = データの整合性と安全を保つため、新しいプロファイルを作成して { $acctEmail } でログインすることをおすすめします。このプロファイルでの同期を続ける場合、両方のアカウントからのデータが “{ $profileName }” 上で永久に統合されます。
