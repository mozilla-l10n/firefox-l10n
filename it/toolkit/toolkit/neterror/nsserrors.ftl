...
ssl-error-handshake-not-completed = Impossibile inizializzare un altro SSL handshake fino a quando l’handshake corrente non è stato completato.
...
ssl-error-unknown-ca-alert = Il peer non riconosce né ritiene attendibile la CA che ha emesso il certificato.
ssl-error-access-denied-alert = Il peer ha ricevuto un certificato valido, ma l’accesso è stato rifiutato.
...
ssl-error-missing-extended-master-secret = Tentativo da parte del peer di ripresa senza una corretta estensione extended_master_secret.
...
sec-error-pkcs12-unable-to-export-key = Impossibile completare l’esportazione. La chiave privata non può essere trovata ed esportata.
...
sec-error-cannot-move-sensitive-key = Una chiave sensibile non può essere spostata dallo slot in cui è necessaria.
...
sec-error-unsupported-ec-point-form = Forma del punto della curva ellittica non supportata.
...
sec-error-incompatible-pkcs11 = Il driver PKCS #11 viola le specifiche creando incompatibilità.
...
sec-error-pkcs11-general-error = Un modulo PKCS #11 ha restituito il valore CKR_GENERAL_ERROR: questo significa che si è verificato un errore irreversibile.
