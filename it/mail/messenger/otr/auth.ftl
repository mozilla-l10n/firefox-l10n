...
auth-helpTitle = Supporto per la verifica

auth-questionReceived = Ecco la domanda posta dal tuo contatto:
...
auth-manualVerification = Verifica manuale fingerprint
auth-questionAndAnswer = Domanda e risposta
auth-sharedSecret = Segreto condiviso
...
auth-manualInstruction = Contattare l’interlocutore desiderato attraverso un canale autenticato diverso, per esempio un’email con firma OpenPGP o via telefono. Scambiarsi fingerprint (un checksum che identifica una chiave crittografica). Se corrisponde, indicare nella finestra di dialogo sottostante che il fingerprint è stato verificato.
