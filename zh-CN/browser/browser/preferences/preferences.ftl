...
content-blocking-and-isolating-etp-warning-description-3 = 此设置可能导致部分网站无法正常使用或显示内容。对于已知可能受您的配置影响的网站，我们提供了可选的例外处理方式。为尽量避免网站出现问题，建议允许对这部分跟踪器进行例外处理。若网站似乎出现问题，您可为其关闭跟踪保护，以加载所有内容；并反馈相关问题，以便我们为全体用户修复。
