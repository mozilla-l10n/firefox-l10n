...
ssl-error-no-trusted-ssl-client-ca = 没有可信任的证书颁发机构用于进行 SSL 客户端认证。
...
mozilla-pkix-error-ca-cert-used-as-end-entity = 该服务器所用的证书使用了基本约束扩展，以此标识它属于证书颁发机构。正常颁发的证书不应如此。
...
mozilla-pkix-error-issuer-no-longer-trusted = 此证书的颁发机构在颁发此证书时已不受信任。
