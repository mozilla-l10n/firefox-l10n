...
csp-xfo-blocked-long-desc = 安全のため、{ -brand-short-name } は他のサイトに埋め込まれた { $hostname } のページの表示を許可できません。このページを表示するには、新規ウインドウで開く必要があります。
...
# This string appears after the following string: "What makes the site look dangerous?" (fp-certerror-why-site-dangerous)
fp-certerror-self-signed-why-dangerous-body = サイトの証明書に問題があります。サイトは証明書認証局により発行された証明書を使用しており、自らが誰であるかを証明していますが、このサイトの証明書は自己署名されています。この証明書は認可された証明書認証局により発行されたものではないため、デフォルトで信頼されません。
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
