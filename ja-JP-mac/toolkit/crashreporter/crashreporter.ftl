...
crashreporter-crashed-and-restore = { -brand-short-name } の動作に問題が発生し、不正終了してしまいました。再起動時にタブとウインドウを可能な限り復元します。
crashreporter-plea = 問題の原因特定と解決のためにご協力いただける方は、クラッシュレポートの送信をお願いします。
crashreporter-information = このアプリケーションは不正終了時に実行され、{ -vendor-short-name } に問題を報告するためのものです。直接単体で起動することはできません。
...
crashreporter-error-failed-to-generate-minidump = アプリケーションがクラッシュ情報の生成に失敗しました。
