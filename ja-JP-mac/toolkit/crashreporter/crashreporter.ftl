...
crashreporter-title = クラッシュレポータ
crashreporter-crash-message = { -brand-short-name } の動作に問題が発生し、不正終了してしまいました。
crashreporter-plea = 問題の原因特定と解決のためにご協力いただける方は、クラッシュレポートの送信をお願いします。
# $details (String) - the reason that a crash report cannot be submitted
crashreporter-error-details = 詳細: { $details }
crashreporter-no-run-message = このアプリケーションは不正終了時にアプリケーションの開発元に問題を知らせるためのものです。直接単体で起動することはできません。
...
crashreporter-report-info = 上記の他にアプリケーション不正終了時の状態についての技術情報がこのレポートに含まれます。
