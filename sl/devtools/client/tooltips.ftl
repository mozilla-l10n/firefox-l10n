...
inactive-css-not-block-container = <strong>{ $property }</strong> nima vpliva na ta element, ker deluje samo na vsebniške elemente tipa block.
inactive-css-not-block-flex-grid-container = <strong>{ $property }</strong> nima vpliva na ta element, ker deluje samo na vsebniške elemente tipov block, flex in grid.
