...
launch-on-login-autostart-infobar-message = { -brand-short-name } so nětko startuje, hdyž so pola Windows přizjewjeće. Móžeće to přeco pozdźišo w nastajenjach změnić.
...
launch-on-login-spotlight-restore-checkbox = Předchadne wokna a rajtarki wočinić
launch-on-login-spotlight-primary-button = { -brand-short-name } składować a začinić
...
nova-early-access-infobar-title = <strong>{ -brand-product-name } nowy napohlad dóstanje.</strong> Widźiće přehlad zažneje, njehotoweje wersije do wozjewjenja pozdźišo w tutym lěće.
nova-early-access-share-feedback-link = Měnjenje dźělić
    .accesskey = M
...
lapsed-user-toast-title = { -brand-product-name } za wami steji
...
refresh-unused-profile-infobar-message = Zda so, zo njejsće { -brand-short-name } chwilku startował. Chceće jón za čerstwe, takrjec nowe dožiwjenje zrumować? Připódla prajene, witajće wróćo!
refresh-reinstalled-profile-infobar-message = Zda so, zo sće { -brand-short-name } znowa instalował. Chceće jón za čerstwe, takrjec nowe dožiwjenje zrumować?
refresh-profile-infobar-button = { -brand-short-name } wobnowić…
    .accesskey = n
