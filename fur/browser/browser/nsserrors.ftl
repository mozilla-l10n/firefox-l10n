...
sec-error-no-krl = Nissun KRL cjatât pal certificât di chest sît.
sec-error-krl-expired = Il KRL dal certificât di chest sît al è scjadût.
sec-error-krl-bad-signature = Il KRL pal certificât di chest sît al à une firme no valide.
sec-error-revoked-key = La clâf pal certificât di chest sît e je stade revocade.
sec-error-krl-invalid = Il KRL gnûf nol à un formât valit.
