...
#   $directive (String): the name of a CSP directive, such as "script-src".
#   $scheme (String): a protocol name, such as "http", which appears as "http:", as it would in a URL.
csp-error-illegal-host-wildcard = { $scheme }: tarzhioù joker e sturiadoù '{ $directive }' a rank enderc'hel un is-domani nann-rumm d'an nebeutañ (sk., *.example.com kentoc'h eget *.com)
