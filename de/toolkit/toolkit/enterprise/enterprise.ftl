...
access-connector-panel-message-error = Die Website ist für die Verwendung zusätzlicher Authentifizierungsmethoden konfiguriert und wird über einen sicheren Unternehmens-Proxy geleitet, aber der { -enterprice-feature-access-connector } ist derzeit nicht verfügbar. Versuchen Sie es später erneut oder kontaktieren Sie Ihren Administrator, wenn das Problem weiterhin besteht.
