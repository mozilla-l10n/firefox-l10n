...
ssl-error-handshake-not-completed = Es kann kein weiterer SSL-Handshake begonnen werden, bis der aktuelle Handshake komplett ist.
...
ssl-error-no-trusted-ssl-client-ca = Keiner Zertifizierungsstelle wird für SSL-Client-Authentifizierung vertraut.
...
ssl-error-no-supported-signature-algorithm = Es wurde kein unterstützter TLS-Signaturalgorithmus konfiguriert.
...
sec-error-adding-cert = Fehler beim Hinzufügen des Zertifikats zur Datenbank.
...
sec-error-cert-no-response = Cert-Bibliothek: Keine Antwort
...
sec-error-pkcs7-keyalg-mismatch = Entschlüsseln ist nicht möglich: Der Schlüssel-Verschlüsselungsalgorithmus stimmt nicht mit Ihrem Zertifikat überein.
...
sec-error-ocsp-unknown-response-type = Die OCSP-Antwort kann nicht vollständig dekodiert werden; sie hat einen unbekannten Typ.
...
sec-error-token-not-logged-in = Der Arbeitsschritt ist fehlgeschlagen, weil das PKCS#11-Token nicht eingeloggt ist.
