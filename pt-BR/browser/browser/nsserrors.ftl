...
ssl-error-rx-malformed-hello-request = O SSL recebeu uma mensagem “Hello Request” defeituosa no processo de handshake.
...
ssl-error-rx-malformed-cert-request = O SSL recebeu uma mensagem “Certificate Request” defeituosa no processo de handshake.
...
ssl-error-rx-unexpected-hello-request = O SSL recebeu uma mensagem “Hello Request” não esperada no processo de handshake.
...
ssl-error-rx-unexpected-cert-request = O SSL recebeu uma mensagem “Certificate Request” não esperada no processo de handshake.
...
ssl-error-rx-malformed-hello-verify-request = SSL recebeu uma mensagem Hello Verify Request malformada.
ssl-error-rx-unexpected-hello-verify-request = SSL recebeu uma mensagem Hello Verify Request inesperada.
...
sec-error-unknown-cert = O certificado solicitado não pôde ser localizado.
...
sec-error-ocsp-malformed-request = O servidor OCSP considerou que a solicitação estava corrompida ou formada incorretamente.
...
sec-error-ocsp-request-needs-sig = O servidor OCSP necessita de uma assinatura para esta solicitação.
sec-error-ocsp-unauthorized-request = O servidor OCSP recusou esta solicitação como não autorizada.
...
sec-error-pkcs11-function-failed = Um módulo PKCS #11 retornou CKR_FUNCTION_FAILED, indicando que a função solicitada não pode ser executada. Tentar de novo a mesma operação pode funcionar.
...
mozilla-pkix-error-v1-cert-used-as-ca = Um certificado X.509 versão 1 que não é confiado foi usado para assinar o certificado deste servidor. Certificados X.509 versão 1 são obsoletos e não devem ser usados para assinar outros certificados.
