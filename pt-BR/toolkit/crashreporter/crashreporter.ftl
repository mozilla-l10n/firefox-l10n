...
crashreporter-no-run-message = Este aplicativo é executado depois de uma falha para informar o problema para o fornecedor do aplicativo. Não deve ser executado diretamente.
...
crashreporter-submit-status = O relatório de falha será enviado antes de você sair ou reiniciar.
...
crashreporter-crash-details = Você pode ver detalhes desta falha em { $url }.
...
crashreporter-error-version-eol = Não há mais suporte a esta versão: relatórios de falha não são mais aceitos.
