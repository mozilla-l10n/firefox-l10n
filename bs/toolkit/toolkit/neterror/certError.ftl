...
fp-certerror-not-yet-valid-why-dangerous-body = Web stranice koriste certifikate koje izdaje tijelo za izdavanje certifikata kako bi dokazale da su zaista ono što tvrde da jesu. { -brand-short-name } ne vjeruje ovoj web-stranici jer izgleda da certifikat neće biti važeći do { DATETIME($date, day: "numeric", month: "numeric", year: "numeric") }.
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
# Variables:
#   $date (Date) - Clock date.
fp-certerror-expired-what-can-you-do-body = Sat na vašem uređaju je postavljen na { DATETIME($date, day: "numeric", month: "numeric", year: "numeric") }. Ako je ovo tačno, sigurnosni problem vjerovatno je sa samom web stranicom. Ako je pogrešno, možete to promijeniti u sistemskim postavkama vašeg uređaja.
# Variables:
#   $error (string) - NSS error code string that specifies type of cert error. e.g. unknown issuer, invalid cert, etc.
