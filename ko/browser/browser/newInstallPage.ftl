...
lost = <b>개인 데이터나 사용자 정의가 없어지지 않았습니다.</b> 이 컴퓨터의 Firefox에 정보를 이미 저장했다면, 다른 Firefox 설치에서도 이 정보를 계속 사용할 수 있습니다.
options-title = 내 선택 사항은 무엇입니까?
options-do-nothing = 아무것도 하지 않으면 { -brand-short-name }의 프로필 데이터가 다른 Firefox 설치의 프로필 데이터와 달라집니다.
options-use-sync = 모든 Firefox 설치에서 모든 프로필 데이터가 동일하도록 하려면 { -fxaccount-brand-name }를 사용하여 동기화 상태를 유지하십시오.
resources = 자원:
