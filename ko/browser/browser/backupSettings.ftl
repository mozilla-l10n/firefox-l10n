...
settings-data-backup-in-progress-message =
    .message = 백업 진행 중…
settings-data-backup-last-backup-date = 최근 백업: { DATETIME($date, timeStyle: "short") }, { DATETIME($date, dateStyle: "short") }
...
restore-from-backup-type-group-label =
    .label = 현재 프로필을 대체하시겠습니까?
restore-from-backup-type-replace =
    .label = 모든 데이터를 이 백업으로 대체하기
restore-from-backup-type-add =
    .label = 데이터 유지 및 새 프로필 생성
restore-from-backup-profiles-disabled-message = 현재 { -brand-short-name }의 모든 데이터를 백업으로 대체합니다.
...
backup-error-retry = 문제가 발생했습니다. 다시 시도해 주세요.
