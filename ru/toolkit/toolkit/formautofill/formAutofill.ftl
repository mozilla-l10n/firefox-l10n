...
credit-card-doorhanger-options-button =
    .title = Настройки автозаполнения форм
...
autofill-insecure-field-warning-description = { -brand-short-name } обнаружил незащищённый сайт. Автозаполнение форм временно отключено.
