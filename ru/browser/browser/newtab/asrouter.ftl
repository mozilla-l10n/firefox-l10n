...
cfr-cookie-banner-handling-body-variant-1 = Позволить { -brand-short-name } автоматически отвечать на уведомления о куках, чтобы вы не отвлекались на них при работе в Интернете. { -brand-short-name } будет отклонять все запросы, если это возможно, или принимать все запросы в обратном случае.
