...
policy-ClearOnShutdown = Удалять данные веб-сёрфинга при закрытии { -brand-short-name }.
...
policy-CNSA2KeyAgreementEnabled = Включить соглашение о ключе CNSA 2.0 ML-KEM-1024 для TLS.
...
policy-DefaultBrowserSettingEnabled = Определяет, может ли пользователь установить { -brand-short-name } в качестве браузера по умолчанию.
