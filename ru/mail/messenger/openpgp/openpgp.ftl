...
key-ring-sign-sub-keys-revoked = Все подключи для подписи ключа { $userId } (идентификатор ключа { $keyId }) отозваны.
key-ring-sign-sub-keys-expired = Срок действия всех подключей для подписи ключа { $userId } (идентификатор ключа { $keyId }) истёк.
