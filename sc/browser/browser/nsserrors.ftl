...
ssl-error-insufficient-security-alert = Su serbidore rechedet tzifrados prus seguros de is suportados dae su cliente.
...
ssl-error-rx-malformed-hello-verify-request = SSL at retzidu unu messàgiu de istabilimentu de connessione (handshake) Hello Verify Request in unu formadu non curretu.
ssl-error-rx-unexpected-hello-verify-request = SSL at retzidu unu messàgiu de istabilimentu de connessione (handshake) Hello Verify Request imprevistu.
ssl-error-feature-not-supported-for-version = Funtzionalidade SSL non cumpatìbile pro sa versione de su protocollu.
ssl-error-rx-unexpected-cert-status = SSL at retzidu unu messàgiu de istabilimentu de connessione (handshake) Certificate Status imprevistu.
ssl-error-unsupported-hash-algorithm = S’àtera parte TLS at impreadu un'algoritmu de hash non cumpatìbile.
ssl-error-digest-failure = Faddina in sa funtzione digest.
ssl-error-incorrect-signature-algorithm = S'est ispetzificadu un'algoritmu de firma non curretu in un'elementu cun firma digitale.
...
ssl-error-next-protocol-no-protocol = Su serbidore no est cumpatìbile cun protocollos decrarados dae su cliente in s’estensione ALPN.
ssl-error-inappropriate-fallback-alert = Su serbidore at refudadu s’istabilimentu de connessione ca su cliente est torradu a una versione de TLS inferiore a cussa cumpatìbile cun su serbidore.
ssl-error-weak-server-cert-key = Sa crae pùblica inclùdida in su tzertificadu de su serbidore est tropu dèbile.
ssl-error-rx-short-dtls-read = Non nch'at bastante ispàtziu in sa memòria intermèdia pro su registru DTLS.
ssl-error-no-supported-signature-algorithm = Non s'est cunfiguradu nissunu algoritmu de firma TLS cumpatìbile.
ssl-error-unsupported-signature-algorithm = S'àtera parte at impreadu una cumbinatzione non cumpatìbile de algoritmu de firma e de hash.
ssl-error-missing-extended-master-secret = S'àtera parte at proadu a sighire chena un’estensione extended_master_secret curreta.
...
sec-error-unknown-issuer = S’autoridade chi donat su tzertificadu de s’àtera parte no est reconnota.
...
sec-error-no-memory = libreria de seguresa: riserva de sa memòria faddida.
...
sec-error-old-krl = Su KLR nou no est imbeniente a su chi ddoe est giai.
...
sec-error-reused-issuer-and-serial = Ses intentende de importare unu tzertificadu cun su pròpiu emitente/nùmeru de sèrie de unu chi esistit giai, ma no est su pròpiu tzertificadu.
...
sec-error-revoked-certificate-ocsp = Su rispondidore OCSP de s’emitente informat chi su tzertificadu est revocadu.
sec-error-crl-invalid-version = Sa lista de rèvoca de tzertificados de s’emitente tenet unu nùmeru de versione disconnotu.
sec-error-crl-v1-critical-extension = Sa lista de rèvoca de tzertificados V1 de s’emitente tenet un’estensione crìtica.
sec-error-crl-unknown-critical-extension = Sa lista de rèvoca de tzertificados V2 de s’emitente tenet un’estensione crìtica disconnota.
sec-error-unknown-object-type = Genia de ogetu disconnotu ispetzificada.
sec-error-incompatible-pkcs11 = Su driver PKCS #11 vìolat s’ispetzificatzione in manera incumpatìbile.
sec-error-crl-already-exists = Su CRL esistet giai.
sec-error-not-initialized = Su NSS no est istadu aviadu.
...
sec-error-pkcs11-general-error = Unu mollu PKCS #11 at torradu su valore CKR_GENERAL_ERROR: custu bolet nàrrere chi est acontèssida una faddina chi non si podet isòrvere.
sec-error-pkcs11-function-failed = Unu mollu PKCS #11 at torradu su valore CKR_FUNCTION_FAILED: custu bolet nàrrere ca sa funtzione rechesta no s’est acumprida. Unu tentativu nou diat pòdere renèssere.
...
mozilla-pkix-error-key-pinning-failure = Su serbidore impreat unu mecanismu de “key pinning” ma non est possìbile creare una cadena de tzertificados de afidamentu chi currispondant a su pinset. No est possìbile inniorare violatziones in contu a su “key pinning”.
mozilla-pkix-error-ca-cert-used-as-end-entity = Su serbidore impreat unu tzertificadu cun un’estensione de is barrancos de base chi dd’identìficant comente autoridade de tzertificatzione. Custu non diat dèpere acontèssere pro unu tzertificadu emìtidu in manera curreta.
...
mozilla-pkix-error-v1-cert-used-as-ca = Pro emìtere su tzertificadu de su serbidore est istadu impreadu unu tzertificadu X.509 versione 1 chi no est de cunfiare. Is tzertificados X.509 versione 1 sunt abbandonados e non si diant dèpere impreare pro firmare àteros tzertificados.
...
mozilla-pkix-error-invalid-integer-encoding = Su serbidore at frunidu unu tzertificadu chi cuntenet una codìfica non vàlida de unu nùmeru intregu. Unas càusas fitianas sunt sa presèntzia de nùmeros de sèrie negativos, mollos RSA o codìficas prus longas de su netzessàriu.
