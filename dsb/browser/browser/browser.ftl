...
browser-shortcut-description = Malsne a priwatne pśeglědowanje
# These are the default window titles everywhere except macOS.
# .data-title-default and .data-title-private are used when the web content
# opened has no title:
#
...
urlbar-identity-button2 =
    .aria-label =
        { $count ->
            [0] Sedłowe informacije pokazaś
            [1] Sedłowe informacije pokazaś (1 pśeslědowak blokěrowany)
            [one] Sedłowe informacije pokazaś ({ $count } pśeslědowak blokěrowany)
            [two] Sedłowe informacije pokazaś ({ $count } pśeslědowaka blokěrowanej)
...
identity-description-tls-key-logging-message = Nałoženje abo słužba móžo waš skoděrowany wobchad z toś togo sedła wiźeś.
...
private-browsing-indicator-button =
    .tooltiptext = Informacije priwatnego modusa
# Title shown in the private browsing info panel.
private-browsing-info-panel-title = Sćo w priwatnem woknje
# Body copy shown in the private browsing info panel. The learn-more link text
# is embedded in the sentence.
private-browsing-info-panel-description = To pomaga, waše pśeglědowanje pśed drugimi na toś tom rěźe schowaś, ale njecyni was online njewidobny. <a data-l10n-name="learn-more">Chto mógł móju aktiwitu wiźeś?</a>
...
urlbar-trust-icon-trackers-blocked-longform-label =
    { $count ->
        [one] { $count } pśeslědowak blokěrowany
        [two] { $count } pśeslědowaka blokěrowanej
        [few] { $count } pśeslědowaki blokěrowane
       *[other] { $count } pśeslědowakow blokěrowane
    }
