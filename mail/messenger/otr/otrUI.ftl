...
start-label = Почати захищену розмову
refresh-label = Оновити захищену розмову
...
# Variables:
#   $name (String) - the screen name of a chat contact person
finger-unseen = Ідентифікатор { $name } ще не підтверджено. Випадкове перехоплення неможливе, але з певними зусиллями хтось може підглядати за вашою розмовою. Запобігайте спостереженню, перевіркою ідентифікатора цього контакту.
# Variables:
#   $name (String) - the screen name of a chat contact person
finger-seen = { $name } зв’язується з вами з невідомого комп’ютера. Випадкове перехоплення неможливе, але з певними зусиллями хтось може підглядати за вашою розмовою. Запобігайте спостереженню, перевіркою ідентифікатора цього контакту.
state-not_private = Поточна розмова не є приватною.
# Variables:
#   $name (String) - the screen name of a chat contact person
state-unverified = Поточна розмова захищена, але не приватна, оскільки ідентифікатор { $name } ще не підтверджено.
# Variables:
#   $name (String) - the screen name of a chat contact person
state-private = Підтверджено ідентифікатор { $name }. Поточна розмова захищена та приватна.
# Variables:
#   $name (String) - the screen name of a chat contact person
